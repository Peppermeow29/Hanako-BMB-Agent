 ...
 * install got into an inconsistent state (a botched manual copy or a
 * self-hosted server pinned to a different source revision). It is NOT a
 * gate; this function only makes an already-running mismatch visible instead
 * of silent. Never call this to decide whether to block or retry anything.
 *
...
export function warnIfServerProtocolMismatch(identity: ServerIdentity, log: (message: string) => void = console.error): void {
  ...
  log(
    `[server-identity] 界面与服务进程版本偏斜：界面期望的运行时协议版本是 ${SERVER_PROTOCOL_VERSION}，`
      + `但当前连接的服务进程返回的是 ${identity.serverProtocol}。这通常发生在手动复制不完整，`
      + '或者连接的是版本不同的自托管服务；仅记录该信息用于排查，不会中断当前运行。',
  );
}
