...
export const ToolGroupBlock = memo(function ToolGroupBlock({ tools: rawTools, collapsed: initialCollapsed, agentName = 'Hanako' }: Props) {
  ...
  const actionPreview = tools.slice(0, 3).map(tool => tool.name).join(' → ');
  const actionSuffix = tools.length > 3 ? ` → +${tools.length - 3}` : '';
      ...
      {!isSingle && (
        ...
        >
          <span className={styles.toolGroupTitle}>
            {summaryText}
            <span className={styles.toolGroupActions} data-tool-name-preview="">{actionPreview}{actionSuffix}</span>
          </span>
