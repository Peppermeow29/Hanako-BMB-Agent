 ...
 *
 * 文件系统或构建工具写入可能让新建文件短暂不可读。本模块在 spawn server 前做
 * 退避检查，把"文件尚未可读"和"依赖确实缺失"区分开。
 *
 * ⚠️ 扩展名必须是 .cjs：根 package.json 有 "type": "module"，.js 会被 Node
 * 当成 ESM，module.exports 失效。同 path-to-file-url.cjs 使用相同的扩展名约束。
 *
...
/**
 * 校验 server/node_modules/ 中关键 external 包是否齐全。
 * 退避重试覆盖常见的文件写入竞态；超过等待窗口仍缺失则当作真缺包。
 *
 ...
 * - maxWaitMs/progressGraceMs: the slow-start guard for cold-start cases.
 *
