...
import {
  installMiniCPM5AgentProfile,
  isMiniCPM5Model,
  normalizeMiniCPM5Model,
} from "./minicpm5-agent-profile.ts";
import {
  createFindTool,
  createGrepTool,
...
export async function createAgentSession(options) {
  ...
  const sessionOptions = !options?.agentDir && typeof resourceLoaderAgentDir === "string" && resourceLoaderAgentDir
    ...
    : options;
  const normalizedOptions = normalizeCreateAgentSessionOptions(sessionOptions);
  // MiniCPM5 uses request-local chat_template_kwargs for its optional think
  // channel.  Keep the persisted/session-level SDK state at `off` so a stale
  // medium/high preference cannot make the first request build an incompatible
  // reasoning prefix; provider compat enables thinking only for explicit
  // math/algorithm turns later.
  const boundedModel = normalizeMiniCPM5Model(normalizedOptions?.model);
  const boundedOptions = boundedModel && boundedModel !== normalizedOptions?.model
    ? { ...normalizedOptions, model: boundedModel }
    : normalizedOptions;
  const sdkOptions = isMiniCPM5Model(boundedOptions?.model)
    && boundedOptions.thinkingLevel !== "off"
    ? { ...boundedOptions, thinkingLevel: "off" }
    : boundedOptions;
  const result = await rawCreateAgentSession(sdkOptions);
  installToolOutcomeAdapter(result?.session);
  installMiniCPM5AgentProfile(result?.session, {
    systemPrompt: sdkOptions?.miniCPM5SystemPrompt,
  });
  installAssistantStreamGuard(result?.session, {
    runtimeSupervisor: sessionOptions?.minicpm5Runtime,
  });
  return result;
}
