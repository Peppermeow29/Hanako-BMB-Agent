...
import { adaptMiniCPM5XmlToolCallStream, sanitizeMiniCPM5Context } from "./minicpm5-stream-adapter.ts";

const STREAM_GUARD_FLAG = Symbol.for("hana.piSdk.streamGuardInstalled");

export function installAssistantStreamGuard(session, guardOptions: any = {}) {
  ...
  agent.streamFn = async (model, context, options) => {
    try {
      const runtime = guardOptions?.runtimeSupervisor;
      if (model?.provider === "minicpm5-local" && typeof runtime?.ensure === "function") {
        await runtime.ensure(model, { signal: options?.signal });
      }
    } catch (error) {
      return createRuntimeErrorStream(model, error);
    }
    const requestContext = model?.provider === "minicpm5-local"
      ? sanitizeMiniCPM5Context({
        ...context,
        // A model switch can leave the SDK's cached full prompt in place. The
        // MiniCPM profile stores the compact prompt on the session and applies
        // it at the final request boundary as a second line of defense.
        ...(typeof session?.__hanaMiniCPM5SystemPrompt === "string"
          ? { systemPrompt: session.__hanaMiniCPM5SystemPrompt }
          : {}),
      })
      : context;
    const inner = await originalStreamFn(model, requestContext, options);
    return guardAssistantMessageStream(adaptMiniCPM5XmlToolCallStream(inner, model, requestContext));
  };
  agent[STREAM_GUARD_FLAG] = true;
}

function createRuntimeErrorStream(model, error) {
  const stream: any = createAssistantMessageEventStream();
  const message = {
    role: "assistant",
    content: [],
    api: model?.api || "openai-completions",
    provider: model?.provider || "minicpm5-local",
...
function recoverInvalidToolCallText(block, bufferedText = "") {
  ...
  // 工具协议碎片：是失败的工具调用，不是 prose，丢弃（不回写成可见文本）。
  if (isToolProtocolFragment(text)) return "";
  return raw;
}
