...
function validateModelMetadata(providerId, model) {
  ...
  if (Object.prototype.hasOwnProperty.call(model, "generation")) {
    const generation = model.generation;
    if (!generation || typeof generation !== "object" || Array.isArray(generation)) {
      throw new ProviderModelMetadataValidationError(providerId, modelId, "generation", "expected an object");
    }
    const numeric = ["temperature", "topP", "minP", "repetitionPenalty"];
    for (const field of numeric) {
      if (!Object.prototype.hasOwnProperty.call(generation, field)) continue;
      const value = generation[field];
      if (typeof value !== "number" || !Number.isFinite(value) || value < 0) {
        throw new ProviderModelMetadataValidationError(providerId, modelId, `generation.${field}`, "expected a finite number greater than or equal to zero");
      }
      if ((field === "topP" || field === "minP") && value > 1) {
        throw new ProviderModelMetadataValidationError(providerId, modelId, `generation.${field}`, "expected a value from 0 to 1");
      }
    }
    if (Object.prototype.hasOwnProperty.call(generation, "topK")) {
      const value = generation.topK;
      if (!Number.isInteger(value) || value < 0) {
        throw new ProviderModelMetadataValidationError(providerId, modelId, "generation.topK", "expected a non-negative integer");
      }
    }
  }
