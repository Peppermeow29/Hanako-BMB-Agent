...
export const PERSISTENT_STORES: readonly StoreDescriptor[] = Object.freeze([
  ...
  defineStore({
    ...
    preCoordinatorReadProjection: {
      ...
      reason: "Desktop startup reads only these named optional fields before the epoch transition; legacy update preferences are ignored by the source-only runtime.",
    },
    siteRules: [
  ...
  defineStore({
    ...
    siteRules: [
      ...rules(["lib/memory/fact-store.ts", "lib/compat/checks/facts-db.ts"], "Opens or repairs the canonical per-agent facts database."),
  ...
  defineStore({
    ...
    siteRules: [
      ...rules(["lib/channels/channel-store.ts"], "Legacy channel storage retained for historical data compatibility."),
  ...
  defineStore({
    ...
    format: "mixed-directory",
    schemaSource: directorySource("desktop/main.cjs", "desktop window bounds and quick-chat window state readers"),
    openEntry: ["desktop window creation"],
    firstPossibleOpenPhase: "runtime_ready",
    firstPossibleWritePhase: "runtime_ready",
    epochPolicy: "compatible",
    ...
    affectedByEpochMigration: false,
    identityContract: "Each fixed path is shell-local presentation metadata, never durable application identity.",
    siteRules: [
      ...rules(["desktop/main.cjs"], "Writes desktop window bounds or quick-chat window state.", ["mkdir", "write-file"], "(?:windowStatePath|quickChatWindowStatePath)"),
    ],
  }),
  defineStore({
