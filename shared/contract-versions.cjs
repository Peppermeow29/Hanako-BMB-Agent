 ...
 * (build scripts, the desktop shell, the server route, tests), and
 ...
 *
 * The server-identity handshake and renderer-side runtime comparison both
 * require this module instead of writing their own literal copy.
 *
 * PRELOAD_API_VERSION — the preload API surface this Electron shell exposes
 * to the renderer. A runtime declaring a higher `contract.preload` than this
 * constant depends on preload capabilities this shell does not have and must
 * be rejected as incompatible.
 *
 * SERVER_PROTOCOL_VERSION — the renderer<->server runtime protocol this
 * build speaks. It is checked at runtime through the existing server-identity
 * handshake so a renderer and server started from different source builds can
 * report a mismatch clearly.
 *
 * Versioning rule (additive-only): bump either number ONLY when new content
 * actually depends on a capability an older shell/server does not have. A
 * source checkout that reports an incompatible number should be rebuilt from
 * the same revision; do not bump casually.
 *
