 ...
 *     专属：本文件只构建源码分享所需的 open composition，不读取任何
 *     发布签名或安装器配置。）
 *
...
import { copyMiniCPM5RuntimeAssets } from "./build-server-runtime-assets.mjs";
...
async function main() {
  ...
  for (const copiedAsset of copyMiniCPM5RuntimeAssets({ rootDir: ROOT, outDir })) {
    console.log(`[build-server-open]   ${copiedAsset}`);
  }
