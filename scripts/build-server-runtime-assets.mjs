...
// MiniCPM5 is an optional local runtime. Package its launchers with the
// server; model weights and the Python/MLX environment remain user-managed.
export const SERVER_RUNTIME_MINICPM5_FILES = [
  "scripts/minicpm5-mlx-server.mjs",
  "scripts/minicpm5_mlx_server.py",
];
...
export function copyMiniCPM5RuntimeAssets({ rootDir, outDir, fsImpl = fs }) {
  const copied = [];
  for (const relativePath of SERVER_RUNTIME_MINICPM5_FILES) {
    const sourcePath = path.join(rootDir, relativePath);
    assertRequiredAssetExists(fsImpl, sourcePath, relativePath);
