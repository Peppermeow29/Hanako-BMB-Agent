...
describe("ModelManager AuthStorage ownership", () => {
  ...
  it("re-applies MiniCPM local tool-use metadata after Pi SDK availability filtering", async () => {
    writeAuth({});
    writeAddedModels({});
    ...
    expect(manager.availableModels.find((model) => (
      model.provider === "minicpm5-local" && model.id === "default_model"
    ))).toMatchObject({
      name: "MiniCPM5-2B (MLX local)",
      input: ["text"],
      contextWindow: 32768,
      maxTokens: 4096,
      toolUse: {
  ...
  it("builds a fresh model credential result from AuthStorage, preserving its dynamic resource URL", async () => {
    writeAddedModels({
      "openai-codex-oauth": {
