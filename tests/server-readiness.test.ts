 ...
 *
 * 场景动机：文件写入或构建工具短暂占用新文件时，server 进程在文件就绪前 spawn
 * 会立刻 ERR_MODULE_NOT_FOUND。这套逻辑在 spawn 前先做退避检查，并能把 stderr
 * 解析成"模块缺失"的可重试信号。
 */
...
describe("isModuleResolutionError", () => {
  ...
  it("issue #484 真实日志 → 抽出 ws", () => {
    // 来自 https://github.com/Peppermeow29/openhanako/issues/484
    const real = [
