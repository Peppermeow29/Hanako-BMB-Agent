...
describe("Windows icon contract", () => {
  it("keeps the Windows app icon as a source asset without installer configuration", () => {
    const pkg = JSON.parse(fs.readFileSync(path.join(ROOT, "package.json"), "utf-8"));
    expect(pkg.build).toBeUndefined();
    expect(fs.existsSync(path.join(ROOT, "desktop", "src", "icon.ico"))).toBe(true);
  });
  ...
  it("loads tray icons from source resources and rejects empty native images", () => {
    const main = fs.readFileSync(path.join(ROOT, "desktop", "main.cjs"), "utf-8");
