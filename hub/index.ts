...
export class Hub {
  ...
  constructor({ engine }) {
    ...
    engine.setHubCallbacks({
      ...
      triggerChannelDelivery: (name, opts) => this.triggerChannelDelivery(name, opts),
      triggerChannelTriage: (name, opts) => this.triggerChannelDelivery(name, opts),
    });
  ...
  triggerChannelDelivery(channelName, opts) {
    throw new Error("Channels are not included in this edition.");
  }
  ...
  _setupDmHandler() {
    // Agent-to-agent phone messaging is retired; bridge messaging is independent.
  }
