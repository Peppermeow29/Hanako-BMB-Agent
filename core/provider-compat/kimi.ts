...
const MOONSHOT_K2_6_CHAT_TEMPERATURE = 1.0;
const MOONSHOT_FIXED_TEMPERATURE_MODELS = new Set(["kimi-k2.6"]);
const KIMI_MODELS_WITHOUT_TEMPERATURE = new Set(["k3", "k3-256k", "kimi-k3"]);
// Moonshot's public K3 endpoint is not the same wire contract as the older
// Kimi Coding K3 aliases: it always reasons and accepts `reasoning_effort`,
// but rejects the K2.x `thinking` request field.
const MOONSHOT_K3_MODELS = new Set(["kimi-k3"]);
...
function usesFixedMoonshotTemperature(model) {
  return lower(model?.provider) === "moonshot"
    && MOONSHOT_FIXED_TEMPERATURE_MODELS.has(lower(model?.id));
}

function moonshotTemperatureForMode(options) {
  return options?.mode === "utility"
    ? KIMI_FOR_CODING_UTILITY_TEMPERATURE
    : MOONSHOT_K2_6_CHAT_TEMPERATURE;
}
...
function isMoonshotK3(model) {
  return lower(model?.provider) === "moonshot"
    && MOONSHOT_K3_MODELS.has(lower(model?.id));
}
...
export function apply(payload, model, options: Record<string, unknown> = {}) {
  ...
  if (usesFixedMoonshotTemperature(model)) {
    // Moonshot K2.6 uses a different fixed temperature for utility calls and
    // streaming Agent requests. Preserve the endpoint's mode-specific wire contract.
    editable().temperature = moonshotTemperatureForMode(options);
  ...
  if (isMoonshotK3(model)) {
    const p = editable();
    // K3 always has preserved reasoning enabled. Sending the K2.x `thinking`
    // field, including `{ type: "disabled" }` from utility calls, is rejected.
    delete p.thinking;
    if (omitsTemperature(model)) delete p.temperature;
