...
4. 只有需要你继续处理的后台任务，系统才会以 <hana-background-result> 消息送达结果。如果没有新结果，不要轮询或臆测完成状态。`
    : `## Background Tasks
...
4. Only background tasks that need your follow-up are delivered via <hana-background-result> messages. If no new result arrives, do not poll or infer completion.`;
}
...
function buildAppendSystemPromptSnapshot({
  ...
  // Keep the 2B model's append prompt bounded. Do not replay AGENTS.md,
  // CLAUDE.md, skills, or deferred-task instructions into its context.
  if (isMiniCPM5Model(targetModel)) {
    return normalizeStringArray([
      workspacePrompt,
      String(locale || "").startsWith("zh")
        ? "MiniCPM5 边缘模式：当前用户指令优先；数学和算法题默认不用工具；只有用户明确给出文件或路径并要求操作时才调用工具。"
        : "MiniCPM5 edge mode: prioritize the current user request; solve math and algorithm questions without tools by default; call a tool only when the user explicitly names a file or path and asks for an operation.",
    ]);
  }
...
export class SessionCoordinator {
  ...
  async createSession(sessionMgr: any, cwd: any, memoryEnabled = true, model: any = null, {
    ...
    let restoredNovelWritingProfile = null;
    if (restore && sessionPathForMeta) {
      try {
        ...
        restoredNovelWritingProfile = normalizeNovelWritingProfile(metaEntry?.novelWritingProfile);
      } catch (err) {
        if (err.code !== "ENOENT") {
    ...
    const restoredPromptModel = restore
    ...
    const promptTargetModel = effectiveModel || restoredPromptModel || unavailableSessionModel || null;
    const miniCPM5Prompt = isMiniCPM5Model(promptTargetModel);
    ...
    const requestedThinkingLevel = restore
      ? (restoredThinkingLevel || this._getDefaultThinkingLevelForModel(promptTargetModel))
      : (thinkingLevel ?? this._getDefaultThinkingLevelForModel(effectiveModel));
    let initialThinkingLevel = normalizeThinkingLevelForModel(
      requestedThinkingLevel || this._getDefaultThinkingLevelForModel(promptTargetModel),
      promptTargetModel,
    );
    let resolvedThinkingLevel = models.resolveThinkingLevel(initialThinkingLevel);
    const providerPromptPatches = promptTargetModel
      ? getProviderPromptPatches(promptTargetModel, {
    ...
    const sessionEntry = {
      ...
      novelWritingProfile: restoredNovelWritingProfile,
    ...
    const systemPromptSnapshot = (restoredPromptSnapshot && !miniCPM5Prompt ? restoredPromptSnapshot.systemPrompt : null)
    ...
    const appendSystemPromptSnapshot = (restoredPromptSnapshot && !miniCPM5Prompt ? restoredPromptSnapshot.appendSystemPrompt : null)
    ...
    const rawSkillsResultSnapshot = miniCPM5Prompt
      ? freezeSkillsResult(null)
      : (restoredPromptSnapshot?.skillsResult
      ?? (
    ...
    const skillsResultSnapshot = miniCPM5Prompt
      ? freezeSkillsResult(null)
      : (restoredPromptSnapshot?.skillsResult
    ...
    const agentsFilesResultSnapshot = miniCPM5Prompt
      ? freezeAgentsFilesResult(null)
      : (restoredPromptSnapshot?.agentsFilesResult
        ?? freezeAgentsFilesResult(baseResourceLoader.getAgentsFiles?.()));
    const promptSnapshotForPersist = restoredPromptSnapshot && !miniCPM5Prompt
    ...
    const targetModelRef = { current: promptTargetModel || null };
    const novelWritingProfileRef = { current: restoredNovelWritingProfile };
    const setNovelWritingProfile = async (profile: any) => {
      const nextProfile = normalizeNovelWritingProfile(profile);
      novelWritingProfileRef.current = nextProfile;
      sessionEntry.novelWritingProfile = nextProfile;
      const activeSessionPath = sessionPathRef.current || sessionPathForMeta;
      if (!activeSessionPath) return;
      await this.writeSessionMeta(activeSessionPath, { novelWritingProfile: nextProfile });
      this._emitSessionMetadataUpdated(activeSessionPath, { novelWritingProfile: nextProfile });
    };
    ...
    const novelWritingExtension = createNovelWritingExtension({
      path: "hana-desktop-novel-writing-profile",
      sessionPathRef,
      getProfile: () => novelWritingProfileRef.current,
      setProfile: setNovelWritingProfile,
    });
    ...
    const resourceLoaderProps = {
      ...
      getExtensions: {
        value: () => {
          ...
          return {
            ...base,
            extensions: [turnContextExtension, novelWritingExtension, visionAuxiliaryExtension, ...(base.extensions || [])],
          };
        },
      },
    ...
    const toolSnapshotOptions: any = { forceMemoryEnabled: frozenMemoryEnabled, model: promptTargetModel || effectiveModel };
    ...
    const {
      ...
      agentToolsSnapshot,
      {
        ...
        agentDir: agent.agentDir,
        model: promptTargetModel || effectiveModel,
        // Sizes the deferred-tool listing against the model this session froze.
        modelContextWindowTokens: (promptTargetModel || effectiveModel)?.contextWindow ?? null,
      },
    );
    const sessionOpts: any = {
      ...
      tools: sessionTools,
      customTools: sessionCustomTools,
      minicpm5Runtime: this._d.getEngine?.()?.getMiniCPM5Runtime?.() || null,
      ...(miniCPM5Prompt ? { miniCPM5SystemPrompt: systemPromptSnapshot } : {}),
    };
    ...
    if (restoredPromptSnapshot?.finalSystemPrompt && !miniCPM5Prompt) {
  ...
  async switchSessionModel(sessionPath: any, newModel: any) {
    ...
    try {
      ...
      if (isMiniCPM5Model(newModel)) {
        const ownerAgent = this._d.getAgentById?.(entry.agentId) || this._d.getAgent?.();
        const compactPrompt = ownerAgent?.buildSystemPrompt?.({ targetModel: newModel });
        if (typeof compactPrompt === "string" && compactPrompt.trim()) {
          session.__hanaMiniCPM5SystemPrompt = compactPrompt;
          session.agent.state.systemPrompt = compactPrompt;
          try { session._baseSystemPrompt = compactPrompt; } catch { /* best effort */ }
        }
  ...
  /** Per-session novel drafting policy. It is intentionally independent from
   * the model's immutable context capability and MLX process-level cache knobs. */
  getNovelWritingProfile(sessionPath = this.currentSessionPath) {
    if (!sessionPath) return null;
    const entry = this._getSessionEntryByPath(sessionPath);
    if (entry && Object.prototype.hasOwnProperty.call(entry, "novelWritingProfile")) {
      return normalizeNovelWritingProfile(entry.novelWritingProfile);
    }
    return normalizeNovelWritingProfile(
      this._readSessionMetaEntrySync(sessionPath)?.novelWritingProfile,
    );
  }
  ...
  async executeIsolated(prompt: any, opts: any = {}) {
    ...
    try {
      ...
      let isolatedPrompt;
      if (isMiniCPM5Model(execModel)) {
        isolatedPrompt = targetAgent.buildSystemPrompt({
          forSubagent: !!opts.subagentContext,
          targetModel: execModel,
        });
      } else if (opts.subagentContext) {
      ...
      const execResourceLoaderProps: any = {
        ...
        getAppendSystemPrompt: {
          value: () => {
            ...
            return buildAppendSystemPromptSnapshot({
              baseAppend: base,
              providerPromptPatches: [],
              hasDeferredResultStore: false,
              locale: targetAgent.config?.locale || getLocale(),
              workspaceScope: execWorkspaceScope,
      ...
      if (isMiniCPM5Model(execModel)) {
        execResourceLoaderProps.getSkills = { value: () => freezeSkillsResult(null) };
      } else if (targetAgent !== agent) {
        execResourceLoaderProps.getSkills = {
          value: () => skills.getSkillsForAgent(targetAgent),
        };
      }
      if (isMiniCPM5Model(execModel)) {
        execResourceLoaderProps.getAgentsFiles = { value: () => freezeAgentsFilesResult(null) };
      }
      ...
      const { session } = await createAgentSession({
        ...
        minicpm5Runtime: this._d.getEngine?.()?.getMiniCPM5Runtime?.() || null,
        ...(isMiniCPM5Model(execModel) ? { miniCPM5SystemPrompt: isolatedPrompt } : {}),
      });
