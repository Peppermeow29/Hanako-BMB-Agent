...
function resolveContextWindow(provider, declared, fallback) {
  const value = declared ?? fallback;
  if (provider !== MINICPM5_PROVIDER_ID) return value;
  const numeric = Number(value);
  if (!Number.isFinite(numeric) || numeric <= 0) return MINICPM5_CONTEXT_WINDOW;
...
function buildModelOverride(modelEntry, modelDefaults = {}, executionHeaders = {}) {
  ...
  if (modelEntry.generation !== undefined) override.generation = modelEntry.generation;
...
function buildModelEntry(
  ...
  const entry: Record<string, any> = {
    ...
    contextWindow: resolveContextWindow(
      provider,
      isObj ? (modelEntry.context ?? modelEntry.contextWindow) : undefined,
      piProtocolBaseline?.contextWindow ?? known?.context ?? DEFAULT_CONTEXT_WINDOW,
    ),
  ...
  if (isObj && modelEntry.generation !== undefined) entry.generation = modelEntry.generation;
