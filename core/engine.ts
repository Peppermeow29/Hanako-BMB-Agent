...
import { applyNovelWritingProviderPayload } from "./novel-writing-profile.ts";
...
function resolveChannelsEnabledForToolAvailability(_engine) {
  return false;
}
...
import { hasMiniCPM5ToolIntent } from "./provider-compat/minicpm5.ts";
...
import { wrapMiniCPM5WorkspaceTools } from "./minicpm5-workspace-boundary.ts";
import { createMiniCPM5RuntimeSupervisor } from "../lib/providers/minicpm5-runtime.ts";

/** Matches the MCP config default; used when no manager config is available. */
const DEFAULT_TOOL_DEFER_THRESHOLD = 10;

// MiniCPM5 runs on the edge with a small tool/prompt budget. Keep the final
// sandboxed surface intentionally narrow even when plugins, MCP, bridge, or
// other optional tools are installed in the host process. Filtering happens
// after sandbox construction so the surviving tools retain the normal
// ResourceIO, sandbox, approval, checkpoint, and cancellation wrappers.
export const MINICPM5_FINAL_TOOL_ALLOWLIST = Object.freeze(new Set([
  "read",
  "write",
  "edit",
  "exec_command",
...
export class HanaEngine {
  ...
  constructor({ hanakoHome, productDir, agentId, appVersion }) {
    ...
    this._miniCPM5Runtime = createMiniCPM5RuntimeSupervisor({
      log: (message) => moduleLog.log(message),
    });
    ...
    // Compatibility-only fields for legacy settings and lifecycle callers.
    // This edition does not construct or load a Computer Use runtime.
  ...
  getProviderCompatOptionsForSession(p) {
    return {
      ...
      novelWritingProfile: this._sessionCoord.getNovelWritingProfile(p),
    };
  }
  /**
  ...
  // Channels are retired from the product surface. Stored channel files and
  // preferences remain readable for compatibility, but execution is disabled.
  getChannelsEnabled() { return false; }
  async setChannelsEnabled(v) { return this._configCoord.setChannelsEnabled(v); }
  isChannelsEnabled() { return false; }
  ...
  getMiniCPM5Runtime() { return this._miniCPM5Runtime; }
  // Compatibility endpoints remain, but this edition ships no GUI executor.
  getComputerHost() { throw new Error("Computer Use is not included in this edition."); }
  getComputerProviders() { throw new Error("Computer Use is not included in this edition."); }
  ...
  async createChannelEntry(input) { throw new Error("Channels are not included in this edition."); }
  async deleteChannelByName(n) { throw new Error("Channels are not included in this edition."); }
  async triggerChannelDelivery(n, o) { throw new Error("Channels are not included in this edition."); }
      ...
      (pi) => {
        ...
        pi.on("before_provider_request", async (event, ctx) => {
          ...
          const requestModel = ctx?.model
            ...
            || null;
          // The extension runner contains hook failures by design. The stream
          // guard performs the same check and turns a startup failure into a
          // normal assistant error visible to the UI; this hook warms the
          // common path before payload serialization finishes.
          if (requestModel?.provider === "minicpm5-local") {
            await this._miniCPM5Runtime.ensure(requestModel, { signal: ctx?.signal });
          }
          const reasoningLevel = this.resolveRequestReasoningLevel(ctx);
          ...
          const normalizedPayload = normalizeProviderPayload(p, requestModel, {
            mode: "chat",
            reasoningLevel,
            deepseekRoleplayReasoningPatch,
            deepseekRoleplayReasoningContext,
          });
          const finalPayload = applyNovelWritingProviderPayload(normalizedPayload, requestModel, novelWritingProfile);
          if (requestModel?.provider === "minicpm5-local" && process.env.MINICPM5_DIAGNOSTICS === "1") {
            const rawTools = Array.isArray(p.tools) ? p.tools : [];
            const finalTools = Array.isArray(finalPayload?.tools) ? finalPayload.tools : [];
            ...
            const latestText = typeof latestUser?.content === "string"
              ...
              : Array.isArray(latestUser?.content)
                ...
                : "";
            moduleLog.log(`[minicpm5-diagnostics] provider_request rawTools=${rawTools.length} finalTools=${finalTools.length}`
              + ` rawNames=${rawTools.map((tool) => tool?.function?.name || tool?.name || "?").join(",")}`
              + ` finalNames=${finalTools.map((tool) => tool?.function?.name || tool?.name || "?").join(",")}`
              + ` intent=${hasMiniCPM5ToolIntent(messages, rawTools)}`
              + ` messages=${messages.length} latestUserChars=${latestText.length}`
              + ` maxTokens=${finalPayload?.max_tokens ?? "none"}`
              + ` thinking=${finalPayload?.chat_template_kwargs?.enable_thinking ?? "none"}`);
          }
          return finalPayload;
        });
      },
  ...
  async dispose() {
    try {
      ...
      try {
        ...
        try {
          await this._miniCPM5Runtime?.close?.();
        } finally {
  ...
  buildTools(cwd, customTools, opts: any = {}) {
    ...
    if (isMiniCPM5Model(opts.model)) {
      const miniCPM5WorkspaceRoot = path.resolve(opts.workspace || cwd);
      result = {
        ...result,
        tools: wrapMiniCPM5WorkspaceTools(
          result.tools.filter((tool) => MINICPM5_FINAL_TOOL_ALLOWLIST.has(tool?.name)),
          miniCPM5WorkspaceRoot,
        ),
        customTools: wrapMiniCPM5WorkspaceTools(
          result.customTools.filter((tool) => MINICPM5_FINAL_TOOL_ALLOWLIST.has(tool?.name)),
          miniCPM5WorkspaceRoot,
        ),
      };
    }
