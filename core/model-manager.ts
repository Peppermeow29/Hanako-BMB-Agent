...
function buildProviderModelMetadataMap(projectionPlans: unknown) {
  ...
  for (const plan of plans) {
    ...
    for (const modelEntry of models) {
      ...
      // Pi's registry strips provider-specific tool contracts from string
      // entries. Reapply the declared contract without changing explicit
      // user model fields such as image:false or visionCapabilities.
      if (meta.toolUse === undefined && known?.toolUse !== undefined) {
        meta.toolUse = structuredClone(known.toolUse);
      }
