...
// MiniCPM5 is an edge model: keep its prompt/tool contract small and
// deterministic. The PI filesystem/command primitives are added later by
// Engine.buildTools; this list controls the agent-owned tools only.
export const MINICPM5_TOOL_ALLOWLIST = Object.freeze(new Set([
  "file",
  "todo_write",
  "stage_files",
]));

function isMiniCPM5Model(model: any) {
  return model?.provider === "minicpm5-local";
}

/**
 * MiniCPM5 的 system prompt 必须与完整 Hana persona 分开维护。
 * 2B 模型把人格、记忆、技能目录和协作说明当成任务上下文时，长对话
 * 会很快退化；这里仅保留当前任务、CodeLife 边界、文本输出和 XML 工具契约。
 */
export function buildMiniCPM5SystemPrompt({ locale = "zh-CN" } = {}) {
  const isZh = String(locale).startsWith("zh");
  if (isZh) {
    return [
      "你是 HanaAgent 中的 MiniCPM5-2B 本地助手。只完成当前用户请求，直接给最终结果；不要编造或猜测。",
      "工作区是 CodeLife。只访问用户当前消息明确提到的路径；未提到路径时不读写文件，也不探索目录。",
      "数学、算法、解释、改写和普通问答默认只用文本。只有用户明确要求工具或文件操作时才调用工具，一次一个；工具返回后再决定是否继续。",
      "工具调用只能用原生 XML：<function name=\"工具名\"><param name=\"参数名\"><![CDATA[参数值]]></param></function>。只能使用当前定义中的工具名和参数，不输出 JSON、Markdown 工具调用或伪造结果。",
      "write 只能写入用户明确指定的路径。工具失败要如实说明，不重复完全相同的调用。",
    ].join("\n\n");
  }
  return [
    "You are the MiniCPM5-2B local assistant inside HanaAgent. Complete only the current user request and answer directly; never invent or guess.",
    "The workspace is CodeLife. Access only paths explicitly named in the current user message; with no path, do not read, write, or explore directories.",
    "For math, algorithms, explanations, rewriting, and ordinary questions, use plain text by default. Call a tool only when the user explicitly requests a tool or file operation, one tool at a time; then use the real result.",
    "Tool calls use native XML only: <function name=\"tool_name\"><param name=\"parameter_name\"><![CDATA[value]]></param></function>. Use only defined names and parameters; never emit JSON, Markdown tool calls, or invented results.",
    "write may target only a path explicitly named by the user. Report tool failures honestly and never repeat an identical call.",
  ].join("\n\n");
}
...
export class Agent {
  ...
  getToolsSnapshot( options: any = {}) {
    ...
    if (!isMiniCPM5Model(options.model)) return snapshot;
    return snapshot.filter((tool) => MINICPM5_TOOL_ALLOWLIST.has(tool.name));
  }
  get tools() {
  ...
  buildSystemPrompt( options: BuildSystemPromptOptions = {}) {
    ...
    const targetModel = Object.prototype.hasOwnProperty.call(options, "targetModel")
      ? options.targetModel
      : null;
    // An explicit target model belongs to the session being built and must
    // win over the agent's persisted default. This matters when a user
    // switches from MiniCPM5 to Kimi without restarting the agent: otherwise
    // the compact edge prompt would leak into the cloud-model session.
    const isMiniCPM5 = Object.prototype.hasOwnProperty.call(options, "targetModel")
      ? isMiniCPM5Model(targetModel)
      : isMiniCPM5Model(this._config?.models?.chat);
    const memoryEnabled = typeof forceMemoryEnabled === "boolean"
      ? forceMemoryEnabled
      : this.memoryEnabled;
    const isZh = String(this.resolveLocale()).startsWith("zh");

    // Do this before loading persona, memory, or user files. Those resources are
    // useful to cloud models but are high-noise context for a 2B edge model.
    if (isMiniCPM5) {
      return buildMiniCPM5SystemPrompt({ locale: isZh ? "zh-CN" : "en-US" });
    }
    ...
    const parts = [
      isZh
        ? "你运行在 HanaAgent 平台上（原名 OpenHanako），由 liliMozi 开发。项目主页：https://github.com/Peppermeow29/openhanako"
        : "You are running on the HanaAgent platform (formerly OpenHanako), developed by liliMozi. Project page: https://github.com/Peppermeow29/openhanako",
    ];
    ...
    // MLX-LM accepts the OpenAI request envelope but does not translate its
    // `tools` field into a native tool-calling prompt. MiniCPM5 therefore
    // needs an explicit, model-facing XML contract in the system prompt; the
    // stream adapter still validates and converts the resulting XML through
    // the normal Pi tool/approval/loop path.
    if (isMiniCPM5) parts.push(isZh
      ? "\n## MiniCPM5 工具调用协议（必须遵守）\n\n" +
        "你当前使用 MiniCPM5-2B 本地模型。工具调用只能输出原生 XML，不能输出 JSON、function_call、工具名称加冒号、Markdown 代码块或自然语言伪造的工具结果。\n" +
        "需要工具时，回复中只放一个完整的 function 元素，然后等待工具结果；一次只能调用一个工具，不能并行或连续拼接多个 function。\n" +
        "固定格式：<function name=\"工具名\"><param name=\"参数名\"><![CDATA[参数值]]></param></function>\n" +
        "无参数工具固定写成：<function name=\"ls\"></function>。例如：<function name=\"ls\"><param name=\"path\"><![CDATA[/tmp/hanako-workspace]]></param></function>。\n" +
        "工具名和参数名必须严格使用当前请求提供的工具定义；不要添加未知参数，不要猜测工具结果。工具返回后，根据真实结果决定是否继续调用下一个工具；不需要工具时才输出普通面向用户的文本。\n" +
        "如果用户没有在本轮明确给出文件名或路径，简单问答必须直接用普通文本回答，禁止调用 read、write、edit、grep、find、ls、file，也禁止用 exec_command 执行 ls/find/grep/cat/pwd 等文件探查；不得从 CodeLife、历史消息或工作区清单自行猜测目标。\n" +
        "当用户同时要求你生成内容并写入文件时，严格分两阶段执行：第一阶段先用普通文本完整输出内容，此阶段禁止调用任何工具，也禁止先 read、ls、find 或检查目标文件；第二阶段只有内容完整输出后，才调用一次 write，把刚才输出的完全相同内容写入目标路径。新文件不存在不是错误，直接 write，不要先 read。写入成功后立即结束或简短确认。\n" +
        "如果内容超过单次输出上限，不要假装已经完成；先明确采用分块方案，按顺序输出并写入，每个后续块只能在前一块成功写入后继续，避免先读不存在的目标文件。\n" +
        "XML 之外不要输出解释、前后缀或 Markdown 标记。参数值一律放进 CDATA；如果没有参数，不要输出空的 param。\n" +
        "只访问当前会话提示中给出的主工作区（当前项目为 CodeLife）；不要用 find、ls、cd 或 exec_command 探索主工作区之外的 Desktop、Home 或其他项目。一次工具返回真实结果后，如果已经满足用户请求，立即用普通文本结束，不要继续试探。"
      : "\n## MiniCPM5 Tool Calling Protocol (MUST FOLLOW)\n\n" +
        "You are running MiniCPM5-2B locally. Tool calls must use native XML only; never emit JSON, function_call, a tool name with a colon, Markdown code fences, or invented tool results.\n" +
        "When a tool is needed, output exactly one complete function element and wait for its result. Call only one tool at a time; never emit parallel or concatenated function elements.\n" +
        "Fixed format: <function name=\"tool_name\"><param name=\"parameter_name\"><![CDATA[value]]></param></function>\n" +
        "For a no-argument tool use: <function name=\"ls\"></function>. Example: <function name=\"ls\"><param name=\"path\"><![CDATA[/tmp/hanako-workspace]]></param></function>.\n" +
        "Tool and parameter names must exactly match the definitions supplied in the current request. Do not add unknown parameters or guess tool results. After a real tool result, decide whether another tool is needed; when no tool is needed, answer the user in ordinary text.\n" +
        "When the user has not explicitly named a file or path in the current turn, answer simple questions in ordinary text without tools. Do not call read, write, edit, grep, find, ls, or file, and do not use exec_command for ls/find/grep/cat/pwd filesystem inspection; never guess a target from CodeLife, earlier messages, or a workspace listing.\n" +
        "When the user asks you to generate content and then save it to a file, follow two strict phases: first output the complete content as ordinary text; during this content phase do not call any tool and do not read, list, find, or inspect the target file first. Only after the content is complete, make one write call containing exactly the content you just output. A missing new file is not an error: write it directly without read. After a successful write, finish or give a brief confirmation.\n" +
        "If the content exceeds one response limit, state that you are using chunks and write them in order; continue each later chunk only after the previous write succeeds, without probing a missing target first.\n" +
        "Only access the primary workspace named in the current session prompt (the current project is CodeLife). Do not explore Desktop, Home, or other projects with find, ls, cd, or exec_command. After one real tool result satisfies the request, finish with ordinary text instead of probing further.\n" +
        "Do not add explanations, prefixes, or Markdown around XML. Put every parameter value inside CDATA; omit param elements for a no-argument call."
    );

    parts.push(isZh
      ? "\n## Session 文件与交付\n\n" +
        (isMiniCPM5
          ? "SessionFile 是与当前 session 相关的本地文件的统一记录：用户上传、你用 write/edit 产生的文件和安装产物都在其中。\n\n"
          : "SessionFile 是与当前 session 相关的本地文件的统一记录：用户上传、你用 write/edit 产生的文件、插件产物、浏览器截图、安装产物都在其中。\n\n") +
        (isMiniCPM5
          ? "- fileId 是机器契约，label 只是展示名；读取和交付优先使用工具返回的 fileId，不要从可见文本猜路径。\n"
        ...
        (isMiniCPM5
          ? "- 需要在命令里使用 session 文件时，先确认工具结果中的可用路径；不要从可见文本回忆或拼接真实路径。\n"
      ...
      : "\n## Session Files and Delivery\n\n" +
        (isMiniCPM5
          ? "SessionFile is the unified record of local files related to the current session: user uploads, files you produce with write/edit, and install outputs.\n\n"
        ...
        (isMiniCPM5
          ? "- fileId is the machine contract; label is display-only. Prefer the fileId returned by a tool for reads and delivery; never guess paths from visible text.\n"
        ...
        (isMiniCPM5
          ? "- When using a session file in a command, use a path returned by the tools; do not recall or reconstruct real paths from visible text.\n"
    ...
    const learnCfg = this._cb?.getLearnSkills?.() || this._config?.capabilities?.learn_skills || {};
    if (!isMiniCPM5 && learnCfg.enabled && learnCfg.allow_github_fetch) {
      parts.push(isZh
        ? "\n## 主动技能获取\n\n" +
    ...
    parts.push(isZh
      ...
      : (isMiniCPM5
        ? "This is the session start time snapshot."
