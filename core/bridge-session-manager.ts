...
function configuredAgentChatModel(agent: any, modelManager: any) {
  const ref = agent?.config?.models?.chat;
  if (!ref?.id || !ref?.provider) return null;
  return findModel(modelManager?.availableModels || [], ref.id, ref.provider);
}
...
export class BridgeSessionManager {
  ...
  _buildOwnerPromptSnapshot(agent, homeCwd, bridgeContext) {
    const targetModel = configuredAgentChatModel(agent, this._deps.getModelManager?.());
    const miniCPM5 = isMiniCPM5Model(targetModel);
    ...
    const workspaceInstructions = miniCPM5 ? "" : buildWorkspaceInstructionPrompt({
      cwd: homeCwd,
    ...
    return this._buildPromptSnapshot(agent, systemPrompt, {
      appendSystemPrompt: [
        ...(miniCPM5 ? [] : (Array.isArray(baseAppend) ? baseAppend : [])),
        ...(workspacePrompt ? [workspacePrompt] : []),
        ...(miniCPM5 ? [String(locale).startsWith("zh")
          ? "MiniCPM5 边缘模式：数学和算法题默认不用工具；只有用户明确给出文件或路径并要求操作时才调用工具。"
          : "MiniCPM5 edge mode: solve math and algorithm questions without tools by default; call a tool only when the user explicitly names a file or path and asks for an operation."] : []),
  ...
  _buildGuestPromptSnapshot(agent, bridgeContext, opts: any = {}) {
    ...
    if (isMiniCPM5Model(targetModel)) {
      ...
      return this._buildPromptSnapshot(
        agent,
        [agent.buildSystemPrompt({ targetModel }), opts.contextTag, bridgePromptLine].filter(Boolean).join("\n\n"),
        {
          appendSystemPrompt: [],
          skillsResult: { skills: [], diagnostics: [] },
          agentsFilesResult: { agentsFiles: [] },
 ...
 * 流中中止（stopReason "aborted"）会正常 resolve 并返回已生成的 partial，error 为 null。
  ...
  async executeExternalMessage(prompt, sessionKey, meta, opts: any = {}) {
    ...
    try {
      ...
      const bridgeModel = configuredAgentChatModel(agent, mm);
      let promptSnapshot = roleChanged || isMiniCPM5Model(bridgeModel)
        ? null
        : normalizeSessionPromptSnapshot(entry.promptSnapshot);
  ...
  _buildOwnerSessionOpts(agent, mm, homeCwd, sessionPathRef = { current: null }, targetModelRef = { current: null }, opts: any = {}) {
    ...
    // Resolve the owner model before assembling tools so model-specific tool
    // surfaces (notably MiniCPM5) apply to bridge sessions as well.
  ...
  async compactSession(sessionKey, opts: any = {}) {
    ...
    const promptSnapshot = freshContext?.promptSnapshot
      || (isMiniCPM5Model(bridgeModel) ? null : restoredPromptSnapshot)
