...
import { MINICPM5_CONTEXT_WINDOW, MINICPM5_PROVIDER_ID } from "../lib/providers/minicpm5-local.ts";
...
function capMiniCPM5Context(providerId, value) {
  if (providerId !== MINICPM5_PROVIDER_ID) return value;
  const numeric = Number(value);
  if (!Number.isFinite(numeric) || numeric <= 0) return value;
  return Math.min(MINICPM5_CONTEXT_WINDOW, Math.floor(numeric));
}

function normalizeMiniCPM5ModelEntry(providerId, model) {
  if (providerId !== MINICPM5_PROVIDER_ID || !isPlainObject(model)) return model;
  const next = { ...model };
  if (next.context !== undefined) next.context = capMiniCPM5Context(providerId, next.context);
  if (next.contextWindow !== undefined) next.contextWindow = capMiniCPM5Context(providerId, next.contextWindow);
  return next;
}
...
import { miniCPM5LocalPlugin } from "../lib/providers/minicpm5-local.ts";
...
export class ProviderRegistry {
  ...
  getChatModelSelection(providerId) {
    ...
    const models = Array.isArray(selectedModels)
      ? cloneData(selectedModels)
        .map((model) => normalizeMiniCPM5ModelEntry(canonicalProviderId, model))
        .filter((model) => getModelType(canonicalProviderId, model) === "chat")
      : [];
    return {
  ...
  getChatDiscoverableModelEntries(providerId) {
    ...
    return mergeProviderModelEntries(
      ...
      .map((model) => normalizeMiniCPM5ModelEntry(sourceProviderId, model))
   ...
   * @param {string} providerId
   * @param {string} modelId
   * @param {{ name?: string, api?: string, context?: number, contextWindow?: number, maxOutput?: number, maxTokens?: number, maxOutputTokens?: number, generation?: object, image?: boolean, video?: boolean, audio?: boolean, reasoning?: boolean, xhigh?: boolean, thinkingLevels?: string[], thinkingLevelMap?: object, defaultThinkingLevel?: string, compat?: object, toolUse?: object, visionCapabilities?: object }} meta
   */
  updateModelEntry(providerId, modelId, meta) {
    ...
    if (meta && typeof meta === "object" && meta.context !== undefined) {
      const cappedContext = capMiniCPM5Context(ownerProviderId, meta.context);
      if (cappedContext !== meta.context) meta = { ...meta, context: cappedContext };
    }

    // 白名单：只允许模型能力字段（image 是标准名，vision 为旧名不写入）
    const ALLOWED = ["name", "api", "context", "maxOutput", "generation", "image", "video", "audio", "reasoning", "xhigh", "thinkingLevels", "thinkingLevelMap", "type", "defaultThinkingLevel"];
    const safe: any = {};
