...
import * as minicpm5 from "./provider-compat/minicpm5.ts";
...
export function normalizeProviderPayload(payload, model, options = {}) {
  ...
  result = applyMiniCPM5LocalGenerationSettings(result, model);
...
/**
 * MLX-LM accepts these OpenAI-compatible sampling fields on every request.
 * Keeping them on the local model metadata makes edits effective immediately
 * and prevents local choices from leaking into cloud provider requests.
 */
export function applyMiniCPM5LocalGenerationSettings(payload, model) {
  if (model?.provider !== "minicpm5-local" || !model?.generation || typeof model.generation !== "object") {
    return payload;
  }
