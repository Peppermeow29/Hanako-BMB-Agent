...
export function classifyHttpRoute({ method = "GET", path = "" } = {}) {
  ...
  if (isSpeechRecognitionRoute(verb, routePath)) return scoped("chat");
...
function isSpeechRecognitionRoute(verb, routePath) {
  if (verb !== "POST") return false;
  return routePath === "/api/media/asr/transcribe";
}
