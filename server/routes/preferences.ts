...
export function createPreferencesRoute(engine: any, options: Record<string, any> = {}) {
  ...
  // Preserve legacy settings URLs; no mutation may resurrect the removed runtime.
  for (const [method, suffix] of [
    ["PUT", ""], ["POST", "/request-permissions"],
    ["POST", "/approvals"], ["DELETE", "/approvals"],
  ]) {
    route.on(method, "/preferences/computer-use" + suffix, (c) => c.json({
      error: "Computer Use is not included in this edition.",
