...
import { isBridgePlatformSupported, bridgePlatformUnavailableMessage } from "../../lib/bridge/platform-support.ts";
import {
...
export function buildBridgeStatus(engine: any, manager: any, agent: any) {
  ...
  const platformStatus = (plat: any, cfg: any, extraFields: any) => {
    const supported = isBridgePlatformSupported(plat);
    const enabled = supported && !!cfg?.enabled;
    const configError = extraFields?.configError || null;
    return {
      ...extraFields,
      enabled,
      supported,
      ...(supported ? {} : { unavailableReason: bridgePlatformUnavailableMessage(plat) }),
      status: !supported ? "disconnected" : enabled && configError ? "error" : live[plat]?.status || "disconnected",
      error: !supported ? bridgePlatformUnavailableMessage(plat) : enabled && configError ? configError : live[plat]?.error || configError,
      agentId: agent.id,
    };
  };
...
export function createBridgeRoute(engine: any, bridgeManagerRef: any) {
  ...
  for (const endpoint of ["config", "owner", "stop", "test", "send-media"]) {
    route.use("/bridge/" + endpoint, async (c, next) => {
      if (c.req.method === "POST") {
        const body = await safeJson(c);
        if (KNOWN_PLATFORMS.includes(body.platform) && !isBridgePlatformSupported(body.platform)) {
          return c.json({ ok: false, code: "FEATURE_UNAVAILABLE", error: bridgePlatformUnavailableMessage(body.platform) }, 410);
        }
      }
      await next();
    });
  ...
  route.post("/bridge/config", async (c) => {
    ...
    const persistedPatch = agent.config?.bridge?.[platform] || patch;
  ...
  route.post("/bridge/test", async (c) => {
    ...
    try {
      ...
      const saved = shouldUseSavedCredentials
        ...
        : {};
      const resolvedCredentials = resolveBridgeCredentials(platform, providedCredentials, saved);
      const effectiveCredentials = shouldUseSavedCredentials ? { ...saved, ...resolvedCredentials } : resolvedCredentials;
      if (platform === "feishu") {
